...
class SpeechPlayerItem: AVPlayerItem {
  ...
  init(session: AudioSession, speechItem: SpeechItem, url: URL) {
...
public class AudioSession: NSObject, ObservableObject, AVAudioPlayerDelegate, CachingPlayerItemDelegate {
  ...
  func updateDuration(forItem item: SpeechItem, newDuration: TimeInterval) {
    if let durations = self.durations, item.idx < durations.count {
      self.durations?[item.idx] = newDuration
    }
  }
  ...
  public func seek(to position: TimeInterval) {
    ...
    if let foundIdx = foundIdx {
      ...
      if let playerItem = player?.currentItem as? SpeechPlayerItem {
        if playerItem.speechItem.idx == foundIdx {
  ...
  public func startAudio() {
    ...
    startStreamingAudio(pageId: pageId)
  }
        ...
        for await speechItem in synthesizer.fetch(from: start) {
          DispatchQueue.main.async {
            ...
            if playWhenReady, self.player?.items().count == 1 {
              self.timer = self.player?.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds(1, preferredTimescale: 600),
                                                                queue: DispatchQueue.main,
                                                                using: self.update)
              self.player?.play()
              self.state = .playing
              self.setupRemoteControl()
              self.update(CMTimeMakeWithSeconds(0, preferredTimescale: 600))
            }
          }
        }
      }
    ...
    if let player = player, player.rate != 0 {
      ...
      case .reset:
        if let playerItem = self.player?.currentItem as? SpeechPlayerItem {
          ...
          timeElapsed = durationBefore(playerIndex: playerItem.speechItem.idx) + itemElapsed
  ...
  var document: SpeechDocument {
    ...
    let result = SpeechDocument(pageId: item!.unwrappedID, wordCount: 10, utterances: utterances)
    return result
  }
}
