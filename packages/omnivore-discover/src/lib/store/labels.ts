...
export const insertLabelToFile = async (
  label: EmbeddedOmnivoreLabel,
): Promise<EmbeddedOmnivoreLabel> => {
  fs.appendFileSync("./newEmbeddings.json", JSON.stringify(label));
...
export const insertLabelToFile$: OperatorFunction<
  EmbeddedOmnivoreLabel,
  EmbeddedOmnivoreLabel
> = mergeMap((x) => fromPromise(insertLabelToFile(x)));
