...
const extractPublishedDateFromAuthor = (author) => {
  ...
  const regex =
    /(January|February|March|April|May|June|July|August|September|Octrober|November|December)\s\d{1,2},\s\d{2,4}/
...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    get okMaybeItsACandidate() {
      return new RegExp(
        `and|(?<!${this.articleNegativeLookAheadCandidates.source})article(?!-(${this.articleNegativeLookBehindCandidates.source}))|body|column|content|^(?!main-navigation|main-header)main|shadow|post-header|hfeed site|blog-posts hfeed|container-banners|menu-opacity|header-with-anchor-widget`,
        'i'
      )
    },

    positive:
      /article|body|content|entry|hentry|h-entry|main|page|pagination|post|text|blog|story|tweet(-\w+)?|instagram|image|container-banners|player/i,
    negative:
...
=======
    videos:
      /\/\/(www\.)?((dailymotion|youtube|youtube-nocookie|player\.vimeo|v\.qq|cdnapisec\.kaltura)\.com|(archive|upload\.wikimedia)\.org|player\.twitch\.tv)/i,
>>>>>>> Stashed changes
  ...
  _checkPublishedDate: function (node, matchString) {
    ...
    let dateRegExpFound = this.REGEXPS.DATES_REGEXPS.find((regexp) =>
      regexp.test(node.textContent.trim())
    )
    dateRegExpFound &&
  ...
  _grabArticle: async function (page) {
    ...
    while (true) {
      ...
      var siblingScoreThreshold = Math.max(
        10,
        ((topCandidate.readability && topCandidate.readability.contentScore) ||
          0) * 0.2
      )
  ...
  _getArticleMetadata: function (jsonld) {
    ...
    var propertyPattern =
      /\s*(dc|dcterm|og|twitter|article)\s*:\s*(locale|author|creator|description|title|site_name|published_time|published|date|image)\s*/gi
